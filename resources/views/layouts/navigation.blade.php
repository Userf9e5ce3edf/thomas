<nav x-data="{ open: false }" class="bg-gray-800 border-b border-gray-700">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @auth
                        @if(Auth::user()->is_admin)
                            <x-nav-link :href="route('interventions.index')" :active="request()->routeIs('interventions.index')" class="text-gray-200">
                                {{ __('All Interventions') }}
                            </x-nav-link>
                            <x-nav-link :href="route('interventions.create')" :active="request()->routeIs('interventions.create')" class="text-gray-200">
                                {{ __('Create Intervention') }}
                            </x-nav-link>
                        @endif
                    @endauth
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <x-dropdown align="right" width="48">
                    ...
                    <x-slot name="content">
                        ...
                        @guest
                            <x-dropdown-link :href="route('login')" class="text-gray-200">
                                {{ __('Login') }}
                            </x-dropdown-link>
                        @endguest
                        @auth
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="route('interventions.index')" :active="request()->routeIs('interventions.index')" class="text-gray-200">
                {{ __('All Interventions') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link :href="route('interventions.create')" :active="request()->routeIs('interventions.create')" class="text-gray-200">
                {{ __('Create Intervention') }}
            </x-responsive-nav-link>
        </div>
        ...
        <div class="mt-3 space-y-1">
            ...
            @endauth
            @guest
                <x-responsive-nav-link :href="route('login')" class="text-gray-200">
                    {{ __('Login') }}
                </x-responsive-nav-link>
            @endguest
        </div>
